...
Status: proposal (not implemented). Complements the Connection API Bridge in
`ConnectionBridge/README.md`, which today proxies browser **HTTP** to Device
...
`href`. That breaks the same network story the Connection bridge solves for
IS-05.

## Design principles (agreed)

1. **Bridge-aware clients remap; do not rewrite Registry JSON** — prefer
   Connection-style canonical public URLs over Lua/`ws_href` response rewriting.
...
- configured Connection Bridge API (or SPA) origin, as `ws` / `wss`
...
`?uri=`). The Connection bridge only maps `urn:x-nmos:control:sr-ctrl/{version}`
...
`href`s; the adapter already receives Device grains. Extend Connection-style
targets:
...
- Product naming once NCP ships ("Connection API Bridge" vs broader Device
  control bridge) — decide at implementation time.
...
- `ConnectionBridge/README.md` — current HTTP bridge and "Query WS not proxied"
